...
const { getLikedReviewIds, setLikeState } = require("../reviewLikeStateService");
...
exports.toggleLikeReview = catchAsync(async (req, res, next) => {
  ...
  await setLikeState(uid, reviewId, isLikedAfterToggle).catch(() => {});
  await deleteByPrefix("feed:");
  if (notificationData) {
...
exports.addComment = catchAsync(async (req, res, next) => {
  ...
  const mentions = extractMentions(text);
    ...
    if (reviewData.userId && reviewData.userId !== uid) {
      engagementRecipients.push({ type: "review_owner", userId: reviewData.userId });
    }

    if (
      parentCommentData?.userId &&
      parentCommentData.userId !== uid &&
      parentCommentData.userId !== reviewData.userId
    ) {
...
exports.getMediaReviews = catchAsync(async (req, res, next) => {
  ...
  const likedIds = await getLikedReviewIds(uid, reviewIds);

  const userCache = await resolveUsernamesFallback(db, snapshot.docs, (d) => d.data());
...
exports.getUserReviews = catchAsync(async (req, res, next) => {
  ...
  const hasMore =
    reviewsSnapshot.docs.length === REVIEW_PAGE_SIZE ||
    listsSnapshot.docs.length === REVIEW_PAGE_SIZE;
  const lastItem = feed[feed.length - 1];
  const nextCursor = lastItem?.createdAt ? getCreatedAtMillis(lastItem.createdAt) : null;
...
exports.getUserReviewsOnly = catchAsync(async (req, res, next) => {
  ...
  const hasMore = snapshot.docs.length === REVIEW_PAGE_SIZE;
