...
const { encryptText } = require("./messageCrypto");
...
const {
  ...
  serializeMessage,
  storagePreview,
} = require("./messages/messageSerializers");
...
async function getConversationOrFail(conversationId, uid) {
  ...
  if (!snapshot || !snapshot.exists()) throw new AppError("Conversa nao encontrada.", 404);

  const conversation = snapshot.val();
  if (!conversation.members?.[uid]) throw new AppError("Voce nao participa desta conversa.", 403);
...
async function createDirectConversation(currentUser, { targetUserId, targetUsername }) {
  ...
  if (target.uid === currentUser.uid) throw new AppError("Voce nao pode abrir conversa consigo mesmo.", 400);
...
async function sendMessage(currentUser, conversationId, { text = "", media = null }) {
  ...
  if (conversation.type === "direct") {
    ...
    await ensureFollowsUser(currentUser.uid, targetUid);
  }
...
async function removeGroupMember(uid, conversationId, memberId) {
  const conversation = await getConversationOrFail(conversationId, uid);
  if (conversation.type !== "group") throw new AppError("Apenas grupos aceitam remocao de membros.", 400);
  if (uid !== memberId && conversation.createdBy !== uid) throw new AppError("Voce nao pode remover este membro.", 403);
  if (!conversation.members?.[memberId]) throw new AppError("Membro nao encontrado no grupo.", 404);
