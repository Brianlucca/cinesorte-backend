...
const resolveTicketRecipientEmail = async (ticket) => {
...
const handleTelegramCommand = async (msg, chatId) => {
  ...
  if (command === "/reply") {
    ...
    try {
      ...
      const recipientEmail = await resolveTicketRecipientEmail(result.ticket);
      const emailResult = await sendSupportTicketReplyEmail({
        protocol: result.ticket.protocol,
        userEmail: recipientEmail,
        userName: result.ticket.name || result.ticket.username,
        subjectLabel: result.ticket.subjectLabel || result.ticket.subject,
        adminResponse: result.ticket.adminResponse,
        threadMessageId: result.ticket.supportThreadMessageId || undefined,
        respondedAt: result.ticket.adminRespondedAt?.toDate
          ? result.ticket.adminRespondedAt.toDate()
          : result.ticket.adminRespondedAt || new Date(),
        conversation: normalizeTicketConversation(result.ticket),
        originalMessage: result.ticket.message || undefined,
      });

      await sendSupportInboxReplyMirrorEmail({
        protocol: result.ticket.protocol,
        userEmail: recipientEmail,
        userName: result.ticket.name || result.ticket.username,
        username: result.ticket.username || null,
        subjectLabel: result.ticket.subjectLabel || result.ticket.subject,
        adminResponse: result.ticket.adminResponse,
        threadMessageId: result.ticket.supportInboxThreadMessageId || undefined,
        respondedAt: result.ticket.adminRespondedAt?.toDate
          ? result.ticket.adminRespondedAt.toDate()
          : result.ticket.adminRespondedAt || new Date(),
        conversation: normalizeTicketConversation(result.ticket),
      ...
      if (emailResult.sent && emailResult.messageId) {
        await result.ticket.ref.update({
          supportThreadMessageId: emailResult.messageId,
        });
        result.ticket.supportThreadMessageId = emailResult.messageId;
      }

      const emailStatus = emailResult.sent
        ? "Email de resposta enviado ao usuário."
        : emailResult.queued
          ? "Email de resposta entrou na fila de reenvio."
          : emailResult.skipped
            ? `Email de resposta não enviado (${emailResult.reason || "sem motivo informado"}).`
            : "Falha ao enviar email de resposta.";
  ...
  if (command === "/close") {
    ...
    try {
      ...
      const recipientEmail = await resolveTicketRecipientEmail(result.ticket);
      const emailResult = await sendSupportTicketClosedEmail({
        protocol: result.ticket.protocol,
        userEmail: recipientEmail,
        userName: result.ticket.name || result.ticket.username,
        subjectLabel: result.ticket.subjectLabel || result.ticket.subject,
        ...
        threadMessageId: result.ticket.supportThreadMessageId || undefined,
        closedAt: result.ticket.closedAt?.toDate ? result.ticket.closedAt.toDate() : result.ticket.closedAt || new Date(),
        conversation: normalizeTicketConversation(result.ticket),
        originalMessage: result.ticket.message || undefined,
      });

      await sendSupportInboxClosedMirrorEmail({
        protocol: result.ticket.protocol,
        userEmail: recipientEmail,
        userName: result.ticket.name || result.ticket.username,
        username: result.ticket.username || null,
      ...
      const emailStatus = emailResult.sent
        ? "Email de encerramento enviado ao usuário."
        : emailResult.queued
          ? "Email de encerramento entrou na fila de reenvio."
          : emailResult.skipped
            ? `Email de encerramento não enviado (${emailResult.reason || "sem motivo informado"}).`
            : "Falha ao enviar email de encerramento.";
