...
async function getUserProfile(uid) {
  if (!uid) throw new AppError("Usuario não informado.", 400);
  const doc = await db.collection("users").doc(uid).get();
  if (!doc.exists) throw new AppError("Usuario não encontrado.", 404);
  return publicUserData(doc.id, doc.data());
}

async function getUserByUsername(username) {
  if (!username) throw new AppError("Usuario não informado.", 400);
  const snapshot = await db.collection("users").where("username", "==", username).limit(1).get();
  if (snapshot.empty) throw new AppError("Usuario não encontrado.", 404);
...
async function ensureFollowsUser(uid, targetUid) {
  ...
  if (!follows) {
    throw new AppError("Voce só pode conversar com pessoas que voce segue.", 403);
  }
}
