...
exports.getUserFollowersList = catchAsync(async (req, res, next) => {
  ...
  res.status(200).json(
    usersDocs
      ...
      .map((doc) => {
        const d = doc.data();
        return { username: d.username, userPhoto: d.photoURL || null, levelTitle: d.levelTitle || null };
      })
  );
});
...
exports.getSuggestions = catchAsync(async (req, res, next) => {
  ...
  const suggestions = snapshot.docs
    ...
    .map((doc) => {
      ...
      return { username: data.username, userPhoto: data.photoURL || null, levelTitle: data.levelTitle || null, score };
    });
