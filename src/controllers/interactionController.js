...
const recordInteraction = async (req, res) => {
  ...
  if (!mediaId || !["like", "dislike", "watched", "favorite"].includes(action)) {
    ...
    await db.runTransaction(async (t) => {
      ...
      if (xpEarned > 0) {
        ...
        if (xpTrophies.length > 0) {
          userUpdates.trophies = admin.firestore.FieldValue.arrayUnion(...xpTrophies);
        }
      }

      if (watchedInc !== 0) {
        const newWatchedCount = (userData.watchedCount || 0) + watchedInc;
        userUpdates.watchedCount = admin.firestore.FieldValue.increment(watchedInc);
        
        const watTrophies = checkTrophies(userData, "watched", newWatchedCount);
        if (watTrophies.length > 0) {
          userUpdates.trophies = admin.firestore.FieldValue.arrayUnion(...watTrophies);
        }
      }
      ...
      if (genreWeight !== 0) {
        try {
          ...
          genres.forEach((g) => {
            userUpdates[`genreCounts.${g.id}`] = admin.firestore.FieldValue.increment(genreWeight);
          });
        } catch (e) {
        }
      }
